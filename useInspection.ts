...
import { inspectComponent } from './api';
...
export function useInspection(): UseInspectionReturn {
  ...
  const inspect = useCallback(async (image: string, mimeType: string, componentType: string, sensitivity: string, instructions: string) => {
    setLoading(true);
    setError(null);
    setResult(null);
    try {
      const data: ApiResponse = await inspectComponent(image, mimeType, componentType, sensitivity, instructions);
      if (!data.success || !data.result) throw new Error(data.error || 'خطای ناشناخته در سرور');
